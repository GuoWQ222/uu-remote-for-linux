...
The legacy version-bound serial installer remains generated by
`packaging/build-deb.sh`; do not hand-edit its copy under `dist/`. New releases
also provide a signed APT repository with separate X11 and GNOME Wayland entry
packages. The archive private key must never enter Git or build artifacts.
Release checksums must cover the shared `.deb`, both entry packages, the
archive-keyring bootstrap package, and any retained installer.
